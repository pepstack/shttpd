 ...
 */
static void show_index(shttpd_arg arg)
{
 ...
 */
static void show_post(shttpd_arg arg)
{
 ...
 */
static void show_secret(shttpd_arg arg)
{
 ...
 */
static void show_huge(shttpd_arg arg)
{
    int state = (int)(uintptr_t)(void*)(arg->state);
    ...
    arg->state = (void*)(uintptr_t)(int)(state);
}
 ...
 */
static void show_404(shttpd_arg arg)
{
    shttpd_printf(arg, "%s", "HTTP/1.1 200 OK\r\n");
    shttpd_printf(arg, "%s", "Content-Type: text/plain\r\n\r\n");
 ...
 */
static void show_users(shttpd_arg arg)
{
    shttpd_printf(arg, "%s", "HTTP/1.1 200 OK\r\n");
    shttpd_printf(arg, "%s", "Content-Type: text/html\r\n\r\n");
    shttpd_printf(arg, "%s", "<html><body>");
    shttpd_printf(arg, "%s", "<h1>Hi. This is a wildcard uri handler for the URI /users/*/ </h1>");
    shttpd_printf(arg, "<h2>URI: %s</h2></body></html>", shttpd_get_env(arg, "REQUEST_URI"));
 ...
 */
static void ssi_test_true(shttpd_arg arg)
{
 ...
 */
static void ssi_test_false(shttpd_arg arg)
{
 ...
 */
static void ssi_print_stuff(shttpd_arg arg)
{
    time_t  t = time(NULL);
...
int main(int argc, char *argv[])
{
     ...
     * Start listening on ports "8888,8889s"
     */
...
#ifndef NO_SSL
    printf("not defined: NO_SSL\n");

    // "shttpd.pem"
    shttpd_set_option(ctx, "ssl_cert", shttpd_get_option(ctx, SHOPT_SSL_CERT));

    // "8888,8889s"
    shttpd_set_option(ctx, "ports", shttpd_get_option(ctx, SHOPT_PORTS));
#endif
